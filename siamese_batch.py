...
class Siamese(object):
    ...
    def detect_image_batch(self, images_1, image_2, batch_size):
        # images_1 是包含32张不同图片的列表，而 image_2 是一张图片
        # img2=jgw img1=sample

        assert len(images_1) == batch_size, "images_1 图片数量不对"
        ...
        for img_1 in images_1:
            img_1 = cvtColor(img_1)
            img_1 = letterbox_image(img_1, [self.input_shape[1], self.input_shape[0]], self.letterbox_image)
            img_1 = preprocess_input(np.array(img_1, np.float32))
            processed_images_1.append(np.expand_dims(np.transpose(img_1, (2, 0, 1)), 0))

        # 预处理 image_2 并复制 32 次
        image_2 = cvtColor(image_2)
        image_2 = letterbox_image(image_2, [self.input_shape[1], self.input_shape[0]], self.letterbox_image)
        image_2 = preprocess_input(np.array(image_2, np.float32))
        image_2 = np.expand_dims(np.transpose(image_2, (2, 0, 1)), 0)
        processed_images_2 = [image_2 for _ in range(batch_size)]  # 复制 image_2 成 32 份

        # 将 32 张图片组合成一个批次
        photo_1 = torch.from_numpy(np.concatenate(processed_images_1, axis=0)).type(torch.FloatTensor)
        photo_2 = torch.from_numpy(np.concatenate(processed_images_2, axis=0)).type(torch.FloatTensor)
        ...
        with torch.no_grad():
            # 获得预测结果，output输出为概率
            output = self.net([photo_1, photo_2])
            output = torch.nn.Sigmoid()(output)

        similarities = []
        for i in range(batch_size):
            similarity = output[i].item()
